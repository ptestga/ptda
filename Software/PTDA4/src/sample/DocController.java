...
public class DocController {
    ...
    @FXML
    private MenuItem menu_receit_trat, menu_sair, menu_logout, menu_sobre;
    @FXML
    private StackedAreaChart<String, Double> chart_freq, chart_pressao;
    @FXML
    private Button btn_receitar_trat, btn_edit_trat, btn_trat_concluir, btn_logout;
    @FXML
    ...
    DocController(String nif, String pass) {
        this.nif = nif;
        this.pass = pass;
    }
    ...
    public void initialize(){
        ...
        //Tratamentos tab
        btn_edit_trat.getStyleClass().addAll("primary");
        btn_receitar_trat.getStyleClass().addAll("primary");
        btn_trat_concluir.getStyleClass().addAll("primary");

        btn_receitar_trat.setOnAction(event -> receitarTrat());

        menu_receit_trat.setOnAction(event -> receitarTrat());

        btn_edit_trat.setOnAction(event -> {
            FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("res/layout/editTrat.fxml"));
            Stage stage = new Stage();

            EditTratController editTratController = new EditTratController(lbl_trat_medico.getText(),
                    lbl_trat_paciente.getText(),lbl_trat_data_inicio.getText(),lbl_trat_data_fim.getText(),
                    textarea_trat_desc.getText(), textarea_trat_notas.getText(), lbl_trat_id.getText());
            fxmlLoader.setController(editTratController);
            ...
            stage.setTitle("Editar tratamento");
            stage.setScene(scene);
            stage.setResizable(false);
            stage.show();
        });
        ...
        list_pacientes.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            ...
            if(Main.getDriver().getTratamentoEstado(idPaciente).equals("s")){

                btn_trat_concluir.setDisable(false);

                btn_trat_concluir.setOnAction(event -> {
                    if(Main.getDriver().finalizarTratamento(idPaciente)) {
                        Main.sucessNotification("Tratamento concluido com sucesso!");
                        btn_trat_concluir.setDisable(true);
                    }else
                        Main.errorNotification("Erro ao concluir o tratamento!");
                });
        ...
        System.out.println("func nif " + nif);
        System.out.println("pass " + pass);
    }

    private void receitarTrat(){
        FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("res/layout/addTrat.fxml"));
        Stage stage = new Stage();

        AddTratController addTratController = new AddTratController(nif);
        fxmlLoader.setController(addTratController);
        ...
        stage.setTitle("Adicionar tratamento");
        stage.setScene(scene);
        stage.setResizable(false);
        stage.show();
    }
